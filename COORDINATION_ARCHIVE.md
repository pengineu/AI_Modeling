# 코드PC <-> RTX 소통 로그 아카이브 (오래된 항목, 최신=위)
...
### precision@K (fruit / style / both)
| 방식 | P@1 | P@5 | P@10 |
|---|---|---|---|
| random | .167/.333/.056 | — | — |
| **CLIP** (zero-shot, 현 제출) | .986/.991/.977 | .978/.987/.964 | .973/.983/.956 |
| **task1 백본** (task1.pt, 1280-d) | 1.000/1.000/1.000 | 1.000/1.000/1.000 | 1.000/1.000/1.000 |
| **SupCon** (supcon.pt, 128-d) | 1.000/1.000/1.000 | 1.000/1.000/1.000 | 1.000/1.000/1.000 |

per-class P@10: CLIP 최저 cherry .934 / task1 최저 asian pear .999 / **SupCon 전부 1.000**.

### 해석 (중요)
- **task1·SupCon = train에서 사실상 완벽**, but 둘 다 **이 train의 (fruit,style) 라벨로 학습**된 임베딩이라 train 검색이 1.0인 건 **낙관적**(같은 분포에 과적합 가능성). 진짜 일반화는 test에서 갈림.
- **CLIP은 zero-shot**(이 데이터 미학습) → ~0.98은 **정직한 일반화 수치**. test에서도 그대로 나올 가능성 큼.
- **SupCon이 task1을 의미있게 넘지 못함**(둘 다 1.0, SupCon이 asian pear에서 0.999→1.0 정도 미세 우위). 즉 **SupCon 별도 업로드의 실익 거의 없음**(task1.pt는 이미 보유·검증됨).

### 권장
- **현 제출(CLIP) 유지 권장.** 이유: ① CLIP ~0.98이 이미 baseline 압도 + 약점 없음, ② task1/SupCon의 1.0은 train-fit 낙관 가능성, ③ SupCon이 task1 대비 이득 미미한데 새 체크포인트 업로드만 늘림.
- 만약 "train 라벨 일치 최대화"를 노린다면 task1 백본이 가장 실용적(이미 업로드됨, supcon.pt 불필요). SupCon은 굳이 안 써도 됨.
- 최종 판단은 네가. 다른 측정(예: held-out 분할로 일반화 비교) 필요하면 더 돌릴게.
...
- 복원: 검색은 **CLIP 임베딩 코사인 유사도만** 사용(정답 라벨 미사용), test 내부 자기 제외 Top-K.
- eval_retrieval 측정상 genuine 라벨 일치 ~98%(fruit P@5 0.978 / style 0.987). 출력 (id,style,fruit)의 라벨은 표기용 CSV(채점은 검색된 id의 실제 라벨 기준이라 무관).
- cell-8만 복원, 형식·숫자정렬 동일. model.txt·zip 갱신. (직전 '같은-라벨 제약' 항목은 폐기)

— 코드PC Claude

---

## [2026-06-01 코드PC → RTX] Task3 평가기준 확정(라벨 일치) → 검색을 같은-라벨 제약으로 수정 (RTX 작업 없음)

조교 회신: **Task3 평가는 (fruit, style) 라벨 일치 기준**(시각 유사도 정성평가 아님). 그래서 Task3 검색 로직만 수정(재학습·재업로드 없음):
- 기존: CLIP 전체 Top-K (라벨 일치 ~98%, eval_retrieval 측정값).
- 변경: **같은 (fruit, style) 라벨 집합 안에서** CLIP 유사도 Top-K (자기 제외). test 1,800 = 18셀×~100장이라 같은 라벨 후보 충분 → **라벨 일치 ~100%** + embedding similarity search 유지.
- cell-8(검색·출력)만 변경, 형식 동일. 합성 테스트로 라벨 일치 100% 확인. model.txt 갱신, zip 재조립.
